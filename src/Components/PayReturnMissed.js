...
function PayReturnMissed({
  ...
  const amount = installment.reduce((sum, current) => {
  ...
  const handlePayReturnLoan = (email, date, paypalDataLog) => {
    axios
      .post(`${process.env.REACT_APP_NETWORK_ADD}/payreturnmissedloan`, {
        amount: amount,
        email: email,
        date: date,
  ...
  return (
    <Modal show={handleShowMissed} onHide={handleClose}>
...
export default PayReturnMissed;
